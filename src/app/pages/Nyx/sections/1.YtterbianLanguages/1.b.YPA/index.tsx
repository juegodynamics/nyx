...
import {Body, Subsection} from 'app/pages/Nyx/Text';
...
>[0]) => {
  ...
  return (
    <Box sx={{width: '100%'}}>
      ...
      >
        {altLetter || <PlayArrow />}
        <audio src={src} ref={ref} onEnded={() => setIsPlaying(false)} />
      </IconButton>
    </Box>
  );
};
    ...
    ({code, shiftKey}) => {
      setFocusedRow((priorFocusedRow) => {
        if (
          ...
          priorFocusedRow === (shiftKey ? 10 : 0) + ROW_SELECTORS[code]
        ) {
          return 0;
        }
              ...
              return (
                ...
                >
                  <TableCell
                    ...
                    component="th"
                    scope="row"
                    {...(focusedConsonant?.manner === manner && {
                  ...
                  {places.map((place, placeIndex) => {
                    const src = consonantAudio?.[manner]?.[place];
                    const isFocusedColumn = focusedColumn - 1 === placeIndex;
                    return (
                      <TableCell
                        key={placeIndex}
                        align="center"
                      ...
                      >
                        {src ? (
                          <Play
                            src={src}
                            isSelected={isFocusedRow && isFocusedColumn}
                            onMouseOver={() =>
                              setFocusedConsonant({place, manner})
                            }
                            onMouseOut={() => setFocusedConsonant(null)}
                            {...(isFocusedRow && {
                              altLetter: COLUMN_DISPLAYERS[placeIndex + 1],
                            })}
                          />
                        ) : null}
                      </TableCell>
